...
for world_id in range(len(WORLDS)):
  ...
  cached_data = read_cached_data(True, dataset_root, targets_file_name=None, output_size=224, Home_name=current_world.encode())
  ...
  ## check if current_world has the target_category
    ...
    if current_world in annotated_targets[target_category].keys():
      ...
      print('** annotated img ids with target', annotated_img_id)
      print('** initial list of image ids', list_init_img_id)

      #print('**', list_init_img_id[1])
      #print('**', annotated_img_id[1])
     
      for idx, init_img_id in enumerate(list_init_img_id):
        if idx >= 10:
          break

        #JK do it for a single path
        if idx >= 1:
          break
        
        #current_img_id = init_img_id
        current_img_id = list_init_img_id[0]
        ...
        ## JK if you want to see a single path
        if flag_one_done > 0:
            break
        ...
        # JK plt.savefig('{}/env_{}_category_{}_id_{}_left.jpg'.format(saved_folder, current_world, target_category, idx), bbox_inches='tight')
        # JK plt.close()
        # print('plotting current image')
        # plt.draw()
        # plt.show()
        # _ = input("first image Press [enter] to continue.")
        ...
        print('** current img', current_img_id)
        print('** target img', target_img_id)

        #shortest path between particular start and target vertex
        ...
        print('** number of visited images', len(list_visited_img_id))
        for j in range(len(path)-1):
          list_actions.append(AVD.action(path[j], path[j + 1]))
        current_img_id = target_img_id
        print('** list visited img ids', list_visited_img_id)
        print('** path', path)
  ...
  ## Evaluation stage
  ## there are more targe views pick the one that has minimal number of steps
        ...
        print('*** one path',current_img_vertex,  target_img_vertex, path)
        print('*** one path', current_img_id, target_img_id, path)
  ...
  ## ======================================================================================================
        ...
        for key, val in all_poses.items():
          x, z, rot, scale = val
          ax1.plot(x, z, color='blue', marker='o', markersize=5)

        #JK debugging just for one path
        flag_one_done = 1
      
        ##  draw the projected target category points
        for i in range(object_points_2d.shape[1]):
          ax1.plot(object_points_2d[0, i], object_points_2d[1, i], color='violet', marker='o', markersize=5)

        ## draw the annotated views
        for img_id in annotated_img_id:
          x, z, rot, scale = all_poses[img_id]
          ax1.plot(x, z, color='yellow', marker='v', markersize=10)
        ##  draw the path:
        xs = []
        zs = []
        for idx, img_id in enumerate(list_visited_img_id):
          ...
          xs.append(x)
          zs.append(z)
          ax1.plot(x,z,color='black', marker='o', markersize=5)
          current_camera_pose = all_poses[img_id]
          current_pose, direction = cameraPose2currentPose(img_id, current_camera_pose, image_structs)
          middle_img_id = img_id
          theta = atan2(direction[2], direction[0])
          # theta = minus_theta_fn(theta, pi/2)
          end_x = x + cos(theta)
          end_z = z + sin(theta)
          sc = 0.5
          ax1.arrow(x, z, sc * cos(theta), sc * sin(theta), head_width=0.3, head_length=0.4, fc='r', ec='r')
          ax1.grid()
          ...
          ax1.plot(xs, zs, color='black', marker='o', markersize=5)
          plt.show()
          plt.draw()
          _ = input("Press [enter] to continue.")

          if flag_target_detected == True:
            print('remove')
            box.remove()
            ax2.patches = []
            flag_target_detected = False
        ...
        # ax2.plot(xs, zs, color='black', marker='o', markersize=5)
        ## draw the start point and end point and middle point
        x, z, rot, scale = all_poses[list_visited_img_id[0]]
        ax1.plot(x, z, color='green', marker='*', markersize=10)
        x, z, rot, scale = all_poses[middle_img_id]
        ax1.plot(x, z, color='cyan', marker='*', markersize=10)
        x, z, rot, scale = all_poses[list_visited_img_id[-1]]
        ax1.plot(x, z, color='red', marker='*', markersize=10)
        ## draw arrow
        x, z, rot, scale = all_poses[middle_img_id]
        theta = atan2(direction[2], direction[0])
        #theta = minus_theta_fn(theta, pi/2)
      ...
      plt.show()
      plt.close()
      # _ = input("oooooo Press [enter] to continue.")
